...
function getProcessor(permalinks = []) {
  return remark()
    .use(wikiLinkPlugin, {
      permalinks,
      pageResolver: (pageName) => [slugify(pageName)],
      hrefTemplate: (permalink) => `/kb/${permalink}`,
    })
    .use(html);
}
  ...
  for (const document of allDocuments) {
    if (document.categorySlug === categorySlug) {
      ...
      if (document.data.isCategoryHome) {
        const processedContent = await getProcessor(allPermalinks).process(document.content);
...
export async function getKbData(slug) {
  ...
  const processedContent = await getProcessor(allPermalinks).process(targetDocument.content);
